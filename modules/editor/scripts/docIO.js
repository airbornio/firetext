	...
	*/
	...
	function getHTML() {
		...
		var doctypeString = doctype ? '<!DOCTYPE '
			...
			+ '>' : '';
		return doctypeString + document.documentElement.outerHTML.replace(/<([a-z]+)[^>]*_firetext_remove=""[^>]*>[^<>]*(?:<\/\1>)?/g, '').replace(/ _firetext_[a-z_]+=""/g, '').replace(/ contenteditable="(?:true|false)"/g, '').replace(/<p[^>]+collab-added=""[^>]+><br><\/p>(<\/body>)/, '$1').replace(/ collab[-a-z]*="[a-z\d.]*"/g, '').replace(/ spellcheck="(?:true|false)"/, '');
	}
